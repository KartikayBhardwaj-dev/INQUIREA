...
from backend.app.tasks.email_tasks import process_email
...
class EmailSyncService:
    ...
    New emails are persisted and automatically queued for intelligence processing via Celery.
    """
    ...
    async def sync_emails(
        ...
        logger.info("Fetched %s Gmail messages for user %s.", len(messages), user.id)

        if not messages:
            return 0

        gmail_ids = [message["id"] for message in messages]

        existing_ids = {
            row[0]
            for row in (
                db.query(Email.gmail_message_id)
                .filter(Email.gmail_message_id.in_(gmail_ids))
                .all()
            )
        }
        ...
        for message in messages:
            ...
            message_data = await gmail.get_email(gmail_message_id)
        ...
        try:
            db.add_all(emails_to_insert)
            db.commit()
        except Exception:
            db.rollback()
            raise

        logger.info("Inserted %s new emails.", len(emails_to_insert))

        for email in emails_to_insert:
            db.refresh(email)

        email_ids = [email.id for email in emails_to_insert]

        # Enqueue background processing tasks after explicit commit
        for email_id in email_ids:
            process_email.delay(email_id)

        logger.info("Queued %s emails for intelligence processing.", len(email_ids))
    ...
    @staticmethod
    def _build_email_model(user_id: int, message_data: dict) -> Email:
        ...
        body = EmailSyncService._extract_body(payload)
        body = EmailSyncService._clean_email_body(body)
